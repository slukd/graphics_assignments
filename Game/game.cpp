...
void Game::WhenRotate(float angle_x, float angle_y)
{
	...
	{
		vector<vector<vector<Shape *>>> new_rubicks_cube = rubicks_cube;
		switch_cube_axis('x', angle_x);
	}
	if (need_to_switch_axis_y)
	{
		vector<vector<vector<Shape *>>> new_rubicks_cube = rubicks_cube;
		switch_cube_axis('y', angle_y);
	}
	if (need_to_switch_axis_z)
	{
		vector<vector<vector<Shape *>>> new_rubicks_cube = rubicks_cube;
		switch_cube_axis('z', angle_z);
	}
}
...
void Game::switch_cube_axis(char axis, float angle)
{
	...
	// Rotate axis and update corresponding rotation angles in the absolute map
	if (axis == 'x')
	{
		// Switch Y and Z axis
		tmp = cube_y_axis;
...
void Game::rotate_cube(float angle, glm::vec3 axis)
{
	...
	{
		switch_cube_axis(rotation_axis, angle);
	}
...
bool Game::rotate_face(float angle, glm::vec3 axis, int index, map<Face, float> &angles_rotated)
{
	...
	// Determine the rotation axis based on the provided axis and the comparison with predefined axis
